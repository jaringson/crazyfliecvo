...
class Animation:
    ...
    def drawAll(self, all_x, all_y, all_z, all_phi, all_theta, all_psi, topics_to_plot, idx):
        # pdb.set_trace()
        ...
        for i, topic in enumerate(topics_to_plot):
            self.drawVehicle(self.meshes[i], all_x[topic][idx], all_y[topic][idx], all_z[topic][idx],
                        all_phi[topic][idx], all_theta[topic][idx], all_psi[topic][idx])
    ...
    def drawVehicle(self, mesh, x, y, z, phi, theta, psi, color="limegreen"):
        # set_trace()
